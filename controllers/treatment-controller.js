...
const CreateTreatment = async (req, res, next) => {
  try {
    ...
    if (treatment.couple && treatment.couple.length > 0) {
      await Promise.all(
        treatment.couple.map(async (element) => {
          ...
          for (let i = 0; i < treatment.durationOfTreatment; i++) {
            ...
            for (let j = 1; j <= treatment.treatmentRecurrancePeriod; j++) {
              const task = new Task({
                ...
                user: treatment.user,
                farm: treatment.farm,
                taskType: "medicalCareTask",
                taskDate: treatmentStartDate,
              });
              await task.save();
              const startOfToday = new Date();
              startOfToday.setMinutes(
                startOfToday.getMinutes() - treatment.timezoneOffset
              );
              startOfToday.setHours(0, 0, 0, 0);
              const startOfTaskDate = new Date(task.taskDate);
              startOfTaskDate.setMinutes(
                startOfTaskDate.getMinutes() - treatment.timezoneOffset
              );
    ...
    if (treatment.bird && treatment.bird.length > 0) {
      await Promise.all(
        treatment.bird.map(async (element) => {
          let treatmentStartDate = new Date(treatment.treatmentStartDate);
