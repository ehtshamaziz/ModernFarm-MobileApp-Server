...
const GetUserCouples = async (req, res, next) => {
  ...
  try {
    ...
    if (req.query.filterClutches === "true") {
      couplesWithClutches = couplesWithClutches.filter(
        (coupleWithClutch) => coupleWithClutch.clutches > 0
      );
    }
  ...
  try {
    ...
    const couplesWithDetails = await Promise.all(
      couples.map(async (couple) => {
        ...
        const fertilityEggs = eggs.filter(
          (egg) => egg.status === "hatched" || egg.status === "fertilized"
        );
        ...
        let avgDeathRate =
          totalDescendants > 0
            ? Math.min(
                ((sumDescendants / totalDescendants) * 100).toFixed(2),
                100
              )
            : 0;
        let avgFertility =
          totalEggs > 0
            ? Math.min(
                ((fertilityEggs.length / totalEggs) * 100).toFixed(2),
                100
              )
            : 0;
        let avgHatching =
          totalEggs > 0
            ? Math.min(
                ((hatchingEggs.length / totalEggs) * 100).toFixed(2),
                100
              )
            : 0;
    ...
    if (req.query.filterClutches === "true") {
      couplesWithDetails = couplesWithDetails.filter(
        (couple) => couple.clutches > 0
      );
    }
...
const ParentCouplesEgg = async (id) => {
  try {
...
const AddCouple = async (req, res, next) => {
  try {
    ...
    if (existingCouple) {
      return res
        .status(300)
        .json({ message: "This male and female bird couple already exists." });
    }
...
const UpdateCouple = async (req, res, next) => {
  try {
    ...
    if (status === "separation") {
      ...
      const maleUpdate = await Bird.findByIdAndUpdate(
        maleBird,
        {
          cageNumber: maleBirdToUpdate.initialCageNumber,
          status: "rest",
