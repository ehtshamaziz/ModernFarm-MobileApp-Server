...
const AddEggs = async (req, res, next) => {
  try {
    ...
    const clutches = await Clutch.findById({ _id: clutch }).populate({
      path: "couple",
      select: "coupleId specie cageNumber user farm",
    });

    const eggs = await Egg.findById(egg._id).populate({
      path: "clutch",
      select: "couple",
      populate: {
        path: "couple",
    ...
    const task = new Task({
      eggId: egg._id,
      user: clutches.couple.user,
      farm: clutches.couple.farm,
      taskType: "fertilityTask",
      taskDate: fertilityDate,
    });
    const task2 = new Task({
      eggId: egg._id,
      user: clutches.couple.user,
      farm: clutches.couple.farm,
      taskType: "hatchingTask",
      taskDate: hatchingDate,
    });
    await task.save();
    await task2.save();
